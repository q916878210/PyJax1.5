...
class Event(threading.Thread):
    ...
    def __init__(self, obj=None, method=None, key=None, interval=0,
    ...
    def __str__(self):
        if self.alive:
            if self.last_run != 0:
                run_time = self.last_run + self.__interval - time.time()
            else:
                run_time = self.start_time + self.__interval - time.time()
        else:
            run_time = 'Dead'
        return '\n--Event--\nkey: %s\nstarted: %s\nlast run: %s\nalive: %s\nobj: %s\n' \
               'method: %s\nOnetime: %s\ninterval: %s\nNext Run: %s\nRun Count: %s\n--End Event--' % \
               (self.key, human_time(self.start_time), human_time(self.last_run), self.alive, self.obj,
                self.method, self.onetime, self.__interval, run_time, self.__run_count)
    ...
    def add_event(event=None, obj=None, method=None, key=None,
                  interval=0, onetime=False, cleanup=False, start=False, responsiveness=1,
                  args=None):
    ...
    def __interrupt_sleep(self):
        for i in range(0, int(self.__interval / self.__responsiveness)):
            if not self.alive:
                return
